...
from services.face_verification.card_face_extractor import CardFaceExtractor
from services.face_verification.selfie_face_extractor import SelfieFaceExtractor
from services.face_verification.face_quality_service import FaceQualityService
from services.face_verification.face_alignment_service import FaceAlignmentService
from services.face_verification.face_embedding_service import FaceEmbeddingService
...
class FaceVerificationService:
    ...
    def verify_faces(
        ...
        """
        Executes complete face verification pipeline.
        """
        all_errors: List[str] = []
        ...
        # Log bounding box and quality details immediately
        self._log_face_metrics(card_bbox, selfie_bbox, card_quality, selfie_quality)

        # Stop early if face detection / extraction failed or face too small
        ...
        # 4. Standardized 5-Point Alignment
        ...
        # 5. Embedding Extraction & L2 Normalization
        ...
        # 7. Decision Logic Gate (MATCH | BORDERLINE | MISMATCH)
    ...
    def _evaluate_decision(
        ...
        """
        Applies configurable 3-tier decision rules:
        - similarity < mismatch_threshold (0.45) -> FACE_MISMATCH
        - mismatch_threshold <= similarity < match_threshold (0.60) -> FACE_SIMILARITY_BORDERLINE (faceVerified=False)
        - similarity >= match_threshold (0.60) -> MATCH (faceVerified=True)
        """
        errs: List[str] = []
