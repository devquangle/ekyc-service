...
def test_enums_serialization():
    assert CardType.CCCD_OLD.value == "CCCD_OLD"
    assert VerificationDecision.MATCH.value == "MATCH"
    assert CrossValidationStatus.MATCH.value == "MATCH"
    assert EkycOutcome.EKYC_VERIFIED.value == "EKYC_VERIFIED"
    assert EkycExecutionStatus.SUCCESS.value == "SUCCESS"

    full_ekyc = FullEkycResponse(
        status=EkycExecutionStatus.SUCCESS,
        ekycResult=EkycOutcome.EKYC_VERIFIED
    )
    d = full_ekyc.model_dump()
    assert d["status"] == "SUCCESS"
    assert d["ekycResult"] == "EKYC_VERIFIED"
